...
import {
  ...
  Users, Package, ShoppingCart, Home, Sliders, CreditCard,
} from "lucide-react";
...
const adminNav = [
  ...
  { href: "/admin/affiliates",   icon: Users,           label: "Affiliés" },
  { href: "/admin/products",     icon: Package,         label: "Produits" },
  { href: "/admin/orders",       icon: ShoppingCart,    label: "Commandes" },
  { href: "/admin/commissions",  icon: CreditCard,      label: "Paiements" },
  { href: "/admin/stats",        icon: BarChart3,       label: "Statistiques" },
  { href: "/admin/platform",     icon: Sliders,         label: "Plateforme" },
...
export default function DashboardLayout({
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
          ...
          <div>
            ...
            {isAdmin && (
              <div className="flex items-center gap-1">
                ...
                <span className="text-[10px] text-amber-400 font-semibold uppercase tracking-wider">PDG</span>
              </div>
            )}
          </div>
        </Link>
      ...
      {/* Back to home (admin only — prominent) */}
      {isAdmin && (
        <div className="px-3 pt-3">
          <Link href="/"
            className="flex items-center gap-2.5 px-3 py-2 rounded-xl text-[13px] font-medium text-white/40 hover:text-white/70 border border-white/[0.05] hover:border-white/[0.1] bg-white/[0.02] hover:bg-white/[0.04] transition-all duration-200">
      ...
      <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
        ...
        {nav.map((item) => {
          ...
          return (
            ...
            >
              {isActive && (
                <div className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-4 bg-[#ff2020] rounded-full" />
              )}
      ...
      <div className="p-3 border-t border-white/[0.04]">
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-white/[0.02] border border-white/[0.04] mb-1">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-white text-[13px] font-medium truncate leading-none mb-0.5">{userName || (isAdmin ? "Admin" : "Affilié")}</div>
            {userEmail && <div className="text-white/30 text-[11px] truncate">{userEmail}</div>}
          </div>
        </div>
