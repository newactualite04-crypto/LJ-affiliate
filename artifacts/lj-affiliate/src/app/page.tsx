...
export default function HomePage() {
  ...
  async function submitAdminPwd(e: React.FormEvent) {
    e.preventDefault();
    setAdminBusy(true);
    setAdminErr("");
    try {
      ...
      if (res.ok) {
        ...
        setAdminErr("Mot de passe incorrect.");
        setAdminBusy(false);
      }
    } catch {
      setAdminErr("Erreur réseau. Réessayez.");
      setAdminBusy(false);
    }
  }
    ...
    <div className="min-h-screen bg-[#0a0a0b] text-white overflow-x-hidden">
      ...
      {adminModal && (
        <div
          ...
          onClick={(e) => { if (e.target === e.currentTarget) { setAdminModal(false); setAdminPwd(""); setAdminErr(""); } }}
        >
          <div
            className="relative w-full max-w-sm rounded-2xl p-7 animate-fade-up"
            style={{
          ...
          >
            ...
            <div className="flex flex-col items-center mb-6">
              <div className="w-11 h-11 rounded-xl bg-[#ff2020]/10 border border-[#ff2020]/20 flex items-center justify-center mb-4">
                <Lock className="w-5 h-5 text-[#ff4040]" />
              </div>
              <h2 className="text-[17px] font-bold text-white tracking-tight">Accès restreint</h2>
              <p className="text-[12px] text-white/35 mt-1 text-center">Entrez le mot de passe administrateur</p>
            </div>

            <form onSubmit={submitAdminPwd} className="space-y-4">
              <div className="relative">
                <input
                  type="password"
                  value={adminPwd}
                  ...
                  autoFocus
                  className="w-full px-4 py-3 rounded-xl text-[14px] text-white placeholder:text-white/25 outline-none transition-all duration-200 font-mono"
                  style={{ background: "rgba(255,255,255,0.04)", border: `1px solid ${adminErr ? "rgba(255,60,60,0.4)" : "rgba(255,255,255,0.08)"}` }}
                />
                {adminErr && (
                  <p className="mt-2 text-[12px] text-red-400">{adminErr}</p>
                )}
              </div>
              ...
              <div className="flex gap-2.5">
                <button
                  type="button"
                  onClick={() => { setAdminModal(false); setAdminPwd(""); setAdminErr(""); }}
                  className="flex-1 py-2.5 rounded-xl text-[13px] font-medium text-white/40 hover:text-white/70 border border-white/[0.07] hover:border-white/[0.14] bg-white/[0.02] transition-all duration-200"
                >
                  Annuler
                </button>
                <button
                  ...
                  className="flex-1 py-2.5 rounded-xl text-[13px] font-semibold text-white transition-all duration-200 disabled:opacity-50"
                  style={{ background: "#ff2020", boxShadow: "0 0 24px rgba(255,32,32,0.35)" }}
                >
                  {adminBusy ? (
